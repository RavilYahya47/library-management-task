...
public class LibraryServiceImpl implements LibraryService {
    ...
    public List<Book> findBooksByAuthor(String authorName) throws SQLException {
        ...
        Optional<Author> matchedAuthor = authors.stream().filter(author -> author.getName().equalsIgnoreCase(authorName)).findFirst();

        if (matchedAuthor.isPresent()) {
            int authorId = matchedAuthor.get().getId();
            return bookDAO.findAll().stream().filter(book -> book.getAuthorId() == authorId).collect(Collectors.toList());
        } else {
    ...
    public List<Book> findAvailableBooks() throws SQLException {
        return bookDAO.findAll().stream().filter(Book::isAvailable).collect(Collectors.toList());
    }
